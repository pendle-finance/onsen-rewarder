...
describe('PendleOnsenRewarder', function () {
  ...
  const setUpDoubleIncentives = async () => {
    ...
    await masterchef.connect(masterchefOwner).set(consts.MASTERCHEF_V2_PID, consts.PENDLE_WETH_ALLOC_POINTS, false);
    const masterchefV2Owner = await impersonateAccount(consts.MASTERCHEF_V2_OWNER);

    await masterchefV2
      .connect(masterchefV2Owner)
      .add(consts.PENDLE_WETH_ALLOC_POINTS, consts.PENDLE_WETH_POOL, rewarder.address);
  ...
  before(async () => {
    ...
    rewarder = await Rewarder.deploy(consts.REWARD_MULTIPLIER, consts.PENDLE_ADDRESS, consts.MASTERCHEF_V2);
    ...
    await lpToken
      ...
      .transfer(alice.address, await lpToken.balanceOf(pendleWethLpWhale.address));
    // TODO:
